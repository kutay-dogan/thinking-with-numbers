...
def perm_diff(s, nA, nB):
    ...
    idx_A = random.sample(all_index, nA)

    idx_B = list(set(all_index) - set(idx_A))
...
def conversion_rate():
    ...
    st.write(
        f"One experiment conv rate difference {perm_diff(balls, 23739, 182 + 22406)}"
    )
...
def anova():
    session = pd.DataFrame.from_dict(
        dict(
...
def web_session_experiment():
    ...
    # st.write(session[session.Page == 'Page B']["Time"].mean())
    true_diff = (
    ...
    st.text("""
            Now let's try to quantify whether Page B is any different than Page A ?
    """)

    st.write(
        f"Observed absolute difference {true_diff:.4f} (or more) probable in {np.mean(np.abs(np.array(perm_diffs)) >= np.abs(true_diff)):.4f} trials out of 1000 trial"
    )
...
def one_shot_experiment(nA, nB, session):
    ...
    all_index = list(range(nA + nB))
    st.write(all_index)
    idx_A = random.sample(all_index, 21)
    st.write(idx_A)
    idx_B = list(set(all_index) - set(idx_A))
    st.write(idx_B)
